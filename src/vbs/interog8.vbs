...
' Check existence of ERALink (both v6 and v4)
set o_fso = CreateObject("Scripting.FileSystemObject")
if o_fso.folderexists("c:\program files\eralink") then
	if o_fso.fileexists("c:\program files\eralink\winteg.exe") then
		s_text = s_text & "ELX," & s_date & "," & s_machine & "," & "ERA Link Std" & vbcrlf
	end if
end if
if o_fso.folderexists("c:\program files\eralink plus") then
	if o_fso.fileexists("c:\program files\eralink plus\winteg.exe") then
		s_text = s_text & "ELX," & s_date & "," & s_machine & "," & "ERA Link Plus" & vbcrlf
	end if
end if
