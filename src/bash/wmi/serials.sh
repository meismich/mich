...
for i in $(< computers.txt ); do
	serial=$( wmic -U apeagers/msadmin% //${i} "select SerialNumber from Win32_BIOS" | tail -1 | sed 's/^[^|]*|\([^|]*\)|.*$/\1/' | sed 's/.*ERROR.*$/ERROR/' )
	if [ "x$serial" == "xERROR" ]; then
		echo "IPAddress ${i} cannot be WMId"
	else
		csname=$( wmic -U apeagers/msadmin% //${i} "select CSName from Win32_OperatingSystem" | tail -1 | sed 's/|.*$//' )
